...
CREATE TABLE `users` (
  ...
  `bookmarked` INTEGER NULL DEFAULT,
  `languages` INTEGER NULL DEFAULT,
  PRIMARY KEY (`id`)
);
...
CREATE TABLE `resources` (
  `id` INTEGER NULL AUTO_INCREMENT DEFAULT,
  `id_creators` INTEGER NULL DEFAULT,
  `id_languages` INTEGER NULL DEFAULT,
  `id_resource_type` INTEGER NULL DEFAULT,
  `link` VARCHAR(255) NOT NULL,
  `date_added` TIMESTAMP NOT NULL,
  `date_created` TIMESTAMP NULL,
  `keywords` VARCHAR(255) NULL,
  `likes` INTEGER NULL DEFAULT,
  `dislikes` INTEGER NULL DEFAULT,
  PRIMARY KEY (`id`)
);
